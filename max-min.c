...
int main()
{
    ...
    for(int k=0;k<tasks-1;k++)
    {
        if(k==0)
        {
            ...
            maxm = *max_element(temp.begin(), temp.end());
            gantt.push_back(maxm);
            row = find_row(maxm,etl,machines,tasks);
            column = find_column(maxm,etl,machines,tasks);
            columns.push_back(column);
            for(int j=0;j<tasks;j++)
            {
                if(j!=column)
                etl[row][j] = etl[row][j] + maxm;
                //cout<<etl[row][j]<<" ";
            }
        }
        else
